...
BEGIN
  ...
  SELECT SUM(row_count) INTO existing_rows FROM (
    ...
    SELECT COUNT(*) FROM rt_job_instance UNION ALL
  ...
  SELECT SUM(invalid_count) INTO invalid_rows FROM (
    SELECT COUNT(*) invalid_count FROM flink_paimon_research.rt_task_version v
      JOIN flink_paimon_research.rt_task t ON t.id=v.task_id AND t.task_type='sync'
      WHERE JSON_VALID(v.config)=0
    UNION ALL
    SELECT COUNT(*) FROM flink_paimon_research.rt_sync_task_config c
      JOIN flink_paimon_research.rt_task t ON t.id=c.task_id AND t.task_type='sync'
      WHERE JSON_VALID(c.config_json)=0
    UNION ALL
    SELECT COUNT(*) FROM flink_paimon_research.rt_job_instance i
      JOIN flink_paimon_research.rt_task t ON t.id=i.task_id AND t.task_type='sync'
    ...
    SELECT COUNT(*) FROM flink_paimon_research.rt_task_operation o
      JOIN flink_paimon_research.rt_task t ON t.id=o.task_id AND t.task_type='sync'
  ...
  WHERE EXISTS (SELECT 1 FROM flink_paimon_research.rt_task t
                WHERE t.task_type='sync' AND t.project_id=p.id);
  ...
  FROM flink_paimon_research.rt_task WHERE task_type='sync';

  INSERT INTO rt_server (id, name, type, address, database_name, database_abbr, account, password, description, operator, create_time, update_time)
  SELECT s.id, s.name, s.type, s.address, s.database_name, s.database_abbr, s.account, s.password,
  ...
  JOIN flink_paimon_research.rt_task t ON t.id=m.task_id AND t.task_type='sync';

  INSERT INTO rt_task_param
  SELECT * FROM flink_paimon_research.rt_task_param WHERE task_type='sync';

  INSERT INTO rt_job_instance
  ...
  FROM flink_paimon_research.rt_job_instance i
  JOIN flink_paimon_research.rt_task t ON t.id=i.task_id AND t.task_type='sync';

  INSERT INTO rt_task_operation
    (id, task_id, job_instance_id, operation_type, operation_status, operator, request_json, result_json,
     error_message, start_time, deadline_at, end_time, active_flag, create_time, update_time)
  SELECT o.id, o.task_id, o.job_instance_id, o.operation_type, o.operation_status, o.operator,
  ...
  SELECT l.* FROM flink_paimon_research.rt_task_change_log l
  JOIN flink_paimon_research.rt_task t ON t.id=l.task_id AND t.task_type='sync';
  ...
  JOIN flink_paimon_research.rt_task t ON a.object_type='task' AND a.object_id=t.id AND t.task_type='sync';

  INSERT INTO rt_paimon_business_domain
  SELECT * FROM flink_paimon_research.rt_paimon_business_domain;

  SELECT COUNT(*) INTO source_rows FROM flink_paimon_research.rt_task WHERE task_type='sync';
  ...
  END IF;
  SELECT COUNT(*) INTO source_rows FROM flink_paimon_research.rt_job_instance i
    JOIN flink_paimon_research.rt_task t ON t.id=i.task_id AND t.task_type='sync';
  SELECT COUNT(*) INTO target_rows FROM rt_job_instance;
  IF source_rows <> target_rows THEN
  ...
  SELECT SUM(invalid_count) INTO invalid_rows FROM (
    ...
    UNION ALL SELECT COUNT(*) FROM rt_job_instance i LEFT JOIN rt_task t ON t.id=i.task_id WHERE t.id IS NULL
  ...
  END IF;
  SELECT COUNT(*) INTO invalid_rows FROM rt_job_instance WHERE managed_flag<>0;
  IF invalid_rows > 0 THEN
...
UNION ALL
SELECT 'orphan_instance', COUNT(*) FROM rt_job_instance i LEFT JOIN rt_task t ON t.id=i.task_id WHERE t.id IS NULL
UNION ALL
SELECT 'managed_import', COUNT(*) FROM rt_job_instance WHERE managed_flag<>0;
