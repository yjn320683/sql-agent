...
export default function RealtimeTablesPage() {
  ...
  const submit = async () => { try {
    ...
    if (new Set(names).size !== names.length) throw new Error('字段名不能重复');
    const propertyNames = (value.extraOptions ?? []).map((item) => item.key?.trim()).filter(Boolean);
    if (new Set(propertyNames).size !== propertyNames.length) throw new Error('表属性不能重复配置');
    if (draft.tableType === 'primary_key' && !columns.some((column) => column.primaryKey)) throw new Error('主键表至少需要一个主键字段');
    if (draft.tableType === 'append_only' && columns.some((column) => column.primaryKey)) throw new Error('Append-only 表不能配置主键');
    setCreating(true);
    await createRealtimeTable({ catalogName: draft.catalogName, databaseName: draft.databaseName!, tableName: draft.tableName!, tableComment: draft.tableComment, comment: draft.tableComment, tableType: draft.tableType!, columns, options: draft.options ?? {} });
    message.success('实时表已创建'); setEditing(false); form.resetFields(); await load();
  } catch (error) { if (error instanceof Error) message.error(error.message); } finally { setCreating(false); } };
  ...
  return <div className="realtime-page realtime-sync-tasks-page realtime-tables-page"><section className="realtime-sync-main-panel">
    ...
    <div className="realtime-sync-table-section"><Table rowKey="id" loading={loading} dataSource={rows} scroll={{ x: 1170 + actionColumnWidth }} pagination={{ current: page, pageSize, total, showSizeChanger: true, pageSizeOptions: [20, 50, 100], showTotal: (value) => `共 ${value} 张` }} onChange={(pagination) => { setPage(pagination.current || 1); setPageSize(pagination.pageSize || 20); }} columns={[
      { title: '实时表', width: 420, fixed: 'left', render: (_, row) => <Button type="link" onClick={() => void openDetail(row.id)}>{row.databaseName}.{row.tableName}</Button> },
      ...
      { title: '生产任务', width: 260, ellipsis: true, render: (_, row) => row.producerTaskName || '-' }, { title: '字段 / 引用', width: 130, render: (_, row) => `${row.columnCount ?? 0} / ${row.referenceCount ?? 0}` },
      { title: '操作', width: actionColumnWidth, fixed: 'right', className: 'table-operation-column', render: (_, row) => <div ref={actionRef(row.id)} className="table-row-actions"><Space><Button type="link" onClick={() => void openDetail(row.id)}>详情</Button><Button type="link" onClick={() => void refresh(row)}>刷新</Button>{row.creationSource === 'manual' && <Button type="link" onClick={() => void openSafeUpdate(row.id)}>安全变更</Button>}</Space></div> },
    ]} /></div>
