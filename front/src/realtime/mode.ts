...
export function storageKeyOf(mode: DevelopmentMode): string {
  return mode === 'realtime' ? LAST_REALTIME_PATH_KEY : LAST_OFFLINE_PATH_KEY;
}

export function fallbackOf(mode: DevelopmentMode): string {
  return mode === 'realtime' ? REALTIME_FALLBACK : OFFLINE_FALLBACK;
}

export function validRememberedPath(mode: DevelopmentMode, remembered?: string | null): string {
